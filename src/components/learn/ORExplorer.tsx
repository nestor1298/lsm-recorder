...
const Hand3DViewer = dynamic(() => import("@/components/Hand3D/Hand3DViewer"), {
  ssr: false,
  loading: () => (
    <div className="flex h-full items-center justify-center rounded-2xl bg-ink">
      <div className="text-center">
        <div className="mx-auto mb-3 h-10 w-10 animate-spin rounded-full border-2 border-accent border-t-transparent" />
        <p className="text-xs text-gray-300">Cargando modelo 3D…</p>
      </div>
    </div>
  ),
});
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl bg-gray-50 p-3">
        <p className="text-xs leading-relaxed text-gray-700">
          La orientación describe <b>hacia dónde mira la palma</b> y{" "}
          <b>hacia dónde apuntan los dedos</b>. Estos dos parámetros definen la
          orientación espacial de la mano en 3D relativa al cuerpo del señante.
        </p>
      </div>
      ...
      <div className="md:hidden">
        <div className="overflow-hidden rounded-2xl bg-ink">
          <Hand3DViewer
            cm={null}
            orientation={{
              palm: orientation.palm,
              fingers: orientation.fingers,
            }}
            autoRotate={false}
            height="280px"
          />
          {/* Orientation info overlay */}
          <div className="border-t border-gray-800/30 bg-black/40 px-4 py-2 text-center">
            <span className="text-xs text-gray-300/80">
              Palma:{" "}
              <b className="text-gray-200">{PALM_LABEL[orientation.palm]}</b>
              {" · "}
              Dedos:{" "}
              <b className="text-gray-200">
                {FINGER_LABEL[orientation.fingers]}
              </b>
            </span>
          </div>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {PALM_DIRECTIONS.map((dir) => (
                <button
                  key={dir.value}
                  onClick={() =>
                    setOrientation((o) => ({ ...o, palm: dir.value }))
                  }
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {FINGER_DIRECTIONS.map((dir) => (
                <button
                  key={dir.value}
                  onClick={() =>
                    setOrientation((o) => ({ ...o, fingers: dir.value }))
                  }
                  className={`rounded-lg px-2 py-2 text-center text-xs font-medium transition-all ${
          ...
          <div className="rounded-xl bg-gray-900 p-3">
            ...
            <p className="font-mono text-sm text-green">
              OR: palma={orientation.palm.toLowerCase()}, dedos=
              {orientation.fingers.toLowerCase()}
            </p>
          </div>
        </div>
        ...
        <div className="hidden md:block">
          <div className="sticky top-24 space-y-2">
            <div className="overflow-hidden rounded-2xl bg-ink">
              ...
              />
              {/* Orientation info overlay */}
              <div className="border-t border-gray-800/30 bg-black/40 px-4 py-2.5">
                <div className="flex items-center justify-between">
                  <span className="text-xs text-gray-300/80">
                    Palma:{" "}
                    <b className="text-gray-200">
                      {PALM_LABEL[orientation.palm]}
                    </b>
                  </span>
                  <span className="text-xs text-gray-300/80">
                    Dedos:{" "}
